  ...
  then
    ...
    case "$(uname)" in
      (OpenBSD)
        ...
        rvm_tar_command=gtar
        ;;
      (Darwin|FreeBSD|DragonFly|Linux) # it's not possible to autodetect on OSX, the help/man does not mention all flags
        rvm_tar_command=tar
        ;;
      (SunOS)
        case "$(uname -r)" in
...
get_and_unpack()
{
  ...
  pwd
  __rvm_debug_command $rvm_tar_command xzf ${rvm_archives_path}/${_file} ${rvm_tar_options:-} ||
  {
    _return=$?
  ...
  shopt -s dotglob
  __rvm_debug_command mv ./rvm-master/* .
  __rvm_debug_command mv ./mpapis-rvm-b1f5ecfafcda/* .
  shopt -u dotglob
  rmdir rvm-master
  rmdir mpapis-rvm-b1f5ecfafcda
}
